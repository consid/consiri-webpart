...
import { css, TextField } from 'office-ui-fabric-react';
...
export default class Consiri extends React.Component<IConsiriWebPartProps, {}> {
  ...
  public render():  React.ReactElement<IConsiriWebPartProps>  {
    return (
  ...
  public componentDidUpdate(prevProps: IConsiriWebPartProps, prevState: {}, prevContext: any): void {
  ...
  public tbKeyDown(e) {
    if (e.keyCode === 13) {
      ...
      const message: Message = {
        type: "message",
        from: { id: this.sendAsUserName },
        text: messageToSend
      }
      this.directLineClient.Conversations.Conversations_PostActivity(
        {
  ...
  protected bindDirectLineSecret(){
    const self = this;
    const { directLineSecret, context } = this.props;
    if (directLineSecret) {
      ...
      this.directLineClientSwagger.then((client) => {
        client.Conversations.Conversations_StartConversation()
          ...
          .then((conversationId) => {
            ...
            self.pollMessages(client, conversationId);
            self.directLineClient = client;
          });
      });

      this.sendAsUserName = context.pageContext.user.loginName;
  ...
  protected pollMessages(client, conversationId) {
    ...
    setInterval(() => {
      client.Conversations.Conversations_GetActivities({ conversationId: conversationId, watermark: watermark })
        ...
        .then((messages) => self.printMessages(messages));
    }, this.pollInterval);
  }

  protected printMessages(messages) {
    if (messages && messages.length) {
      messages = messages.filter((m) => m.from.id !== this.sendAsUserName);
      if (messages.length) {
